...
describe('SafeServices', () => {
  describe('estimateSafeTx', async () => {
    it('should throw an error', async () => {
      ...
      await chai.expect(service.estimateSafeTx("some_safe_address", safeTx)).to.be.rejectedWith("Test error")
      chai.expect(mockNetwork.post).to.have.been.calledWith(
    ...
    it('should return estimate', async () => {
      ...
      await chai.expect(service.estimateSafeTx("some_safe_address", safeTx)).to.be.eventually.deep.equals(BigNumber.from(424242))
      chai.expect(mockNetwork.post).to.have.been.calledWith(
        "some_base_url/api/v1/safes/some_safe_address/multisig-transactions/estimations/",
        safeTx
      )
  ...
  describe('getSafeTxDetails', async () => {
    it('should throw an error', async () => {
      ...
      await chai.expect(service.getSafeTxDetails("some_safe_tx_hash")).to.be.rejectedWith("Test error")
      chai.expect(mockNetwork.get).to.have.been.calledWith(
        "some_base_url/api/v1/multisig-transactions/some_safe_tx_hash"
      )
    })
    ...
    it('should return details', async () => {
      ...
      await chai.expect(service.getSafeTxDetails("some_safe_tx_hash")).to.be.eventually.deep.equals("this needs to be replace with proper data")
      chai.expect(mockNetwork.get).to.have.been.calledWith(
        "some_base_url/api/v1/multisig-transactions/some_safe_tx_hash"
      )
    })
