...
- viem 2.52 tightened its ABI types to the `0x${string}` template-literal type. The SDK keeps its public API on plain `string` (via the abitype `AddressType` override) and casts to `0x${string}` only at the viem call boundaries, so the address-typed surface is unchanged.
