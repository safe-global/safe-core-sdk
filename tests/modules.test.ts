...
describe('Safe modules', () => {
  ...
  describe('buildContractCall ENABLE_MODULE', async () => {
    ...
    it('should fail if address is equal to sentinel', async () => {
      ...
      const tx = safeSdk.buildContractCall({
        method: SafeSettings.ENABLE_MODULE,
        moduleAddress: SENTINEL_MODULES
      })
      await chai.expect(tx).to.be.rejectedWith('Invalid module address provided')
    })
    ...
    it('should fail if address is already enabled', async () => {
      const { safe, module1 } = await setupTests()
      const safeSdk = new EthersSafe(ethers, safe.address, user1)
      const tx1 = await safeSdk.buildContractCall({
        method: SafeSettings.ENABLE_MODULE,
        moduleAddress: module1.address
      })
      const txResponse = await safeSdk.executeTransaction(tx1, { gasLimit: 10000000 })
      await txResponse.wait()
      const tx2 = safeSdk.buildContractCall({
        method: SafeSettings.ENABLE_MODULE,
        moduleAddress: module1.address
      })
  ...
  describe('buildContractCall DISABLE_MODULE', async () => {
    it('should fail if address is invalid', async () => {
      const { safe } = await setupTests()
      const safeSdk = new EthersSafe(ethers, safe.address, user1)
      const tx = safeSdk.buildContractCall({
    ...
    it('should disable Safe modules', async () => {
      ...
      const tx2 = await safeSdk.buildContractCall({
        method: SafeSettings.ENABLE_MODULE,
        moduleAddress: module2.address
      })
      ...
      const tx3 = await safeSdk.buildContractCall({
        method: SafeSettings.DISABLE_MODULE,
        moduleAddress: module1.address
      })
      ...
      const tx4 = await safeSdk.buildContractCall({
        method: SafeSettings.DISABLE_MODULE,
        moduleAddress: module2.address
      })
