...
- 4aaa5d2: Improve `baseGas` accuracy in `estimateTxBaseGas`. The estimator now models several costs it previously omitted or approximated:
...
- 5611215: chore: bump safe deployments
...
- e5aaecc: Fix numeric `blockTag` in `SafeProvider.getBalance`, `getNonce`, `getContractCode`, `isContractDeployed`, and `call`, which previously threw when given a number.
- Updated dependencies [69d6047]
- Updated dependencies [69d6047]
- Updated dependencies [614242e]
