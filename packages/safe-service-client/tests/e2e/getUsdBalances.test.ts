...
describe('getUsdBalances', () => {
  ...
  it('should return the list of USD balances', async () => {
    ...
    chai.expect(usdcBalance.fiatCode).to.be.equal('USD')
    chai.expect(usdcBalance.fiatBalance).not.to.be.equal('')
    chai.expect(usdcBalance.fiatConversion).not.to.be.equal('')
  })
