...
import SafeApiKit, { SafeDelegateConfig } from '@safe-global/api-kit/index'
...
describe('getSafeDelegates', () => {
  ...
  it('should return an array of delegates', async () => {
    ...
    await safeApiKit.addSafeDelegate(delegateConfig1)
    await safeApiKit.addSafeDelegate(delegateConfig2)
    const safeDelegateListResponse = await safeApiKit.getSafeDelegates(safeAddress)
    ...
    await safeApiKit.removeAllSafeDelegates(safeAddress, signer)
  })
  ...
  it('should return an array of delegates EIP-3770', async () => {
    ...
    await safeApiKit.addSafeDelegate(delegateConfig1)
    await safeApiKit.addSafeDelegate(delegateConfig2)
    const safeDelegateListResponse = await safeApiKit.getSafeDelegates(eip3770SafeAddress)
    const { results } = safeDelegateListResponse
    const sortedResults = results.sort((a, b) => (a.delegate > b.delegate ? -1 : 1))
    chai.expect(sortedResults.length).to.be.eq(2)
    ...
    chai.expect(sortedResults[1].delegator).to.be.eq(await delegateConfig2.signer.getAddress())
    chai.expect(sortedResults[1].label).to.be.eq(delegateConfig2.label)
    await safeApiKit.removeAllSafeDelegates(eip3770SafeAddress, signer)
  })
})
