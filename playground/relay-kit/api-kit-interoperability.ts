...
// Constants
const BUNDLER_URL = `https://api.pimlico.io/v2/${CHAIN_NAME}/rpc?add_balance_override&apikey=${PIMLICO_API_KEY}`;
const PAYMASTER_URL = `https://api.pimlico.io/v2/${CHAIN_NAME}/rpc?add_balance_override&apikey=${PIMLICO_API_KEY}`;
...
async function main() {
	...
	const safeOpSignedByOwner2 =
		await safe4337Pack.signSafeOperation(addedSafeOperation);
	...
	const confirmedSafeOperation =
		await apiKit.getSafeOperation(safeOperationHash);
